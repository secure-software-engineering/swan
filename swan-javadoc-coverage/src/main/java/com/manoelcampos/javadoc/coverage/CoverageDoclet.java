...
import com.manoelcampos.javadoc.coverage.exporter.ConsoleSummaryExporter;
...
public class CoverageDoclet {
    ...
    public CoverageDoclet(final RootDoc rootDoc) {
        ...
        this.exporter = new ConsoleSummaryExporter(this, getSourcePath());
    }
    ...
    private String getSourcePath() {
        for (final String[] option : rootDoc.options()) {
            if (option.length == 2 && option[0].equals("-sourcepath")) {
                return Utils.includeTrailingDirSeparator(option[1]);
            }
        }
