...
public class ParameterInCallFeature extends AbstractSootFeature {
  ...
  public ParameterInCallFeature(String cp, String methodName,
      CheckType checkType) {
    this(cp, methodName, -1, false, checkType);
  }

  public ParameterInCallFeature(String cp, String methodName,
      boolean restrictPackage, CheckType checkType) {
    this(cp, methodName, -1, restrictPackage, checkType);
  }

  public ParameterInCallFeature(String cp, String methodName,
      int parameterPostion, boolean restrictPackage, CheckType checkType) {
    super(cp);
      ...
      while (!params.equals(oldParams)) {
        ...
        for (Unit u : body.getUnits()) {
          ...
          if (this.checkType == CheckType.CheckSource
            ...
            if (argumentPosition == -1 && inv.getMethod().getName().startsWith(this.methodName)) params.add(assi.getLeftOp());
            }
            }
          if (this.checkType == CheckType.CheckFromInterfaceToResult)
            if (u instanceof AssignStmt) {
            AssignStmt assi = (AssignStmt) u;
            if (assi.containsInvokeExpr()) if (assi.getInvokeExpr().getMethod().isAbstract() || assi.getInvokeExpr().getMethod().getDeclaringClass().isInterface()) params.add(assi.getLeftOp());
            }
          ...
          if (stmt.containsInvokeExpr()) {
            ...
            if (this.checkType == CheckType.CheckSink
              ...
              if (!isTransformer(methodSink)) if (inv.getMethod().getName().startsWith(methodSink) || inv.getMethod().getDeclaringClass().getName().startsWith(methodSink)) if (checkPackageRestriction(sm.getDeclaringClass(), inv.getMethod().getDeclaringClass())) for (Value vInv : inv.getArgs())
